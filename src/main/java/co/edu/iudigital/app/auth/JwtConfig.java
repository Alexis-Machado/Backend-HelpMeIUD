...
// Configura una clave secreta para firmar y verificar la integridad del JWT.
// Si no se proporciona una clave secreta, se generará automáticamente por defecto (visible en JwtAccessTokenConverter).

// Utiliza el algoritmo RSA para generar estas claves, que deben ser colocadas en AuthorizationServer.
// La firma del token se realiza con la clave privada, y la verificación de su validez e integridad se efectúa con la clave pública.

// Aunque hay varias herramientas disponibles, en este contexto se recomienda el uso de openssl.
// Comandos de openssl:
// openssl genrsa -out jwt.pem -> para generar la clave privada.
// openssl rsa -in jwt.pem -> para visualizar la clave privada.
// openssl rsa -in jwt.pem -pubout -> para mostrar la clave pública.

// Estas claves desempeñan un papel crucial en el cifrado y verificación del token.

// jwt.io permite la visualización del contenido del token.
